...
if __name__ == "__main__":
    ...
    sys.stdout.flush()
    name = sys.argv[1]
    metrics = [EMetric(0.5,name+"_e"),SegmentationMetrics(10,0.5,name)]
    print(model)
    ...
    for i in range(10): # XXX 
        if i % 100 == 0:
            print(f'4,9,{i}')
            sys.stdout.flush()
        analyse_event(tf.constant([4,9,i]),model,metrics)

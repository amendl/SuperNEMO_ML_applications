...
task            = import_arbitrary_module("task","/sps/nemo/scratch/amendl/AI/my_lib/optimizing_midle/scripts/dataset.py")
...
if __name__=="__main__":
    ...
    with strategy.scope():
        ...
        dataset = tf.data.Dataset.from_generator(
            generator = lambda: task.generator([4],[0,1,2,3,4,5,6,7],events),
        ...
        val_dataset = tf.data.Dataset.from_generator(
            generator = lambda: task.generator([4],[8],events),
            output_signature=(tf.TensorSpec(shape=(3),dtype=tf.int32))
        )
    ...
    history = model.fit(
        x = dataset.batch(256).prefetch(2),
        epochs = 10,
        validation_data = val_dataset.batch(256).prefetch(2)
    )

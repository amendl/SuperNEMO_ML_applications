...
void TKEvent::add_tracker_hit(TKtrhit* hit)
{
...
{
	reconstruct_multi_from_hits(tr_hits, save_sinograms);
}

void TKEvent::reconstruct_multi_from_hits(std::vector<TKtrhit*> hits, bool save_sinograms)
{
	auto tr_hits = hits;
...
void TKEvent::make_top_projection(std::string path)
{
	gROOT->SetBatch(true);
